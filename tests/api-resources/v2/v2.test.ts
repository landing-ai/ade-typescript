...
function stubClient(handler: (url: string) => Response): {
  ...
  return { client, calls, sentForm: () => sentBody as FormData };
}
...
describe('client.v2 routing', () => {
  ...
  test('an options array is rejected on both the object and the string branch', async () => {
    // `coerceOptions` validates the object branch too, so a JS caller cannot smuggle
    // an array in past the string check and have it spread into `{"0": ...}`.
    const { client } = stubClient(() => jsonResponse({ markdown: 'x', metadata: {} }));
    for (const options of [['x'] as unknown as Record<string, unknown>, '["x"]', '[["password","sneaky"]]']) {
      await expect(
        client.v2.parse({
          document: await toFile(Buffer.from('%PDF'), 'locked.pdf'),
          options,
  ...
  test('an options string that is not JSON is rejected, not smuggled top-level', async () => {
    // This used to fall back to a top-level `password` form field. No snapshot
    // since 2026-07-13 declares one, so the gateway drops it and the caller loses
    // the key to a 422 that names nothing. Reject the malformed `options` instead
    // -- the request was already unsendable, and the error says which field.
    const { client } = stubClient(() => jsonResponse({ markdown: 'x', metadata: {} }));
    await expect(
      client.v2.parse({
        document: await toFile(Buffer.from('%PDF'), 'locked.pdf'),
