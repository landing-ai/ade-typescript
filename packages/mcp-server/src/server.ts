...
export { ClientOptions } from 'landingai-ade';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/ade',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the ade MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new LandingAIADE({
    ...{ environment: (readEnv('LANDINGAI_ADE_ENVIRONMENT') || undefined) as any },
    logger,
