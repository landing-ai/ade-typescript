...
const prompt = `Runs JavaScript code to interact with the LandingAI ADE API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { VISION_AGENT_API_KEY: requireValue(readEnv('VISION_AGENT_API_KEY') ?? client.apikey, 'set VISION_AGENT_API_KEY environment variable or provide apikey client option'), LANDINGAI_ADE_BASE_URL: readEnv('LANDINGAI_ADE_BASE_URL') ?? readEnv('LANDINGAI_ADE_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = 'landingai-ade';
