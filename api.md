...
`client.v2.parseJobs`, `client.v2.extractJobs`, and `client.v2.groundJobs` all return a single, unified <a href="./src/resources/v2/types.ts">`Job`</a> shape even though the underlying parse/extract/ground job envelopes differ upstream — `Job.raw` retains the full original envelope as an escape hatch.
