...
Every node in a parse `structure` tree carries a <a href="./src/resources/v2/types.ts">`V2Grounding`</a> (`{ page, range, box }`, plus an optional `min_ocr_confidence`). Word-granularity models (`dpt-3-fast`) populate `min_ocr_confidence` at every level with one weakest-link rule: a word `atomic_grounding` entry carries the lowest per-character OCR confidence in that word, and each parent grounding (element, `table_cell`, `table`, page) carries the lowest confidence among the words below it. It is omitted wherever no transcribed word carries a score — line-granularity models (`dpt-3-pro`), blocks whose text the model wrote rather than read (captioned figures and similar), and blocks with markdown suppressed — so test it with `== null`. The older `confidence` name for the same score is deprecated and still declared on `V2Grounding` for backwards compatibility; read `min_ocr_confidence`.

Both grounding numbers arrive at a fixed precision: a <a href="./src/resources/v2/types.ts">`V2GroundingBox`</a> coordinate carries at most 5 decimal places and `min_ocr_confidence` at most 2. The gateway clamps and rounds before serializing, so these are the stored values, not a truncation of something finer.
