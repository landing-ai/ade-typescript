...
Every node in a parse `structure` tree carries a <a href="./src/resources/v2/types.ts">`V2Grounding`</a> (`{ page, range, box }`, plus an optional `confidence`). `confidence` is scored per **word** and only there: word-granularity models (`dpt-3-verity`) populate it on each `atomic_grounding` entry, where it is the lowest per-character OCR confidence in that word. It is omitted on node-level grounding — the `grounding` of a page, element, `table_cell`, or `table` — and on line-granularity models (`dpt-3-pro`), so test it with `== null`. There is no rolled-up score on a parent: to judge a node, take the minimum over the segments below it.

A grounding also carries an optional `address`: for spreadsheet input only, the Excel-style reference of the content it covers, sheet name included — `Sales!C5` for a cell, `Sales!C5:F20` for a table, the anchor cell (`Sales!B2`) for content parsed out of an embedded image. It is omitted for page-based documents, so test it with `== null`. The node a parsed workbook emits per sheet carries that sheet's name in the optional `id` on <a href="./src/resources/v2/types.ts">`V2ParsePage`</a>, which is likewise omitted on a page node. Prefer `address` over an element `id` as a durable key: `V2ParseElement.id` is an **opaque** string — the contract no longer documents a `<type>-<index>` format, so do not parse it — and it is stable only within a single response, whereas an address is stable across re-parses of the same file.

Note one gap the SDK has not closed: the contract now documents `Grounding.page` and `Grounding.box` as `null` for a workbook (a sheet has no page number and no visual position) and `V2ParsePage.type` as `page | sheet`, but all three stay at their existing non-nullable, `page`-only types here, because widening them is a breaking change for every caller that reads them. Code that may see spreadsheet input should guard `page` and `box` with `== null` before dereferencing.
...
`client.v2.extract` and `client.v2.extractJobs.create` take `strict` and `grounding`. Both are hand-written top-level shorthands: the contract declares them inside the nested `options` object, which neither method surfaces, and `ade-python` exposes the same two with the same semantics (CONTRIBUTING.md pins that parity). `grounding: false` skips the grounding stage, so every `extraction_metadata` leaf comes back with `ranges: null` and the request finishes faster — Preview, and with grounding off the `extraction` itself can differ slightly from a grounded run (empty leaves are not nulled, all-empty array rows are not dropped). Omitting either one — or passing `null` — leaves its key out, so the server default applies (`strict` false, `grounding` true).
