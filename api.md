...
`client.v2.parseJobs.list` and `client.v2.extractJobs.list` take the page size as `pageSize` — the spec renamed that query parameter from `page_size`, so the older <code><a href="./src/resources/v2/parse.ts">V2JobListParams</a></code> property of that name is deprecated and no longer read by the gateway (the page size falls back to its default of 10). The rename is request-side only: the returned <a href="./src/resources/v2/types.ts">`JobList`</a> still reports `page_size`. The extract listing also declares `cancelled` in its status enum now, so expect that <a href="./src/resources/v2/types.ts">`JobStatus`</a> from `client.v2.extractJobs.list`.
